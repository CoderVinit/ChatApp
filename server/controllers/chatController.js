...
const newGroupChat = TryCatch(async (req, res, next) => {
  ...
  // if (members.length < 2) {
  //   return next(new ErrorHandler('More than 2 members are required to form a group chat', 401))
  // }

  const chat = await Chat.find({ name });

  if (chat) {
    return next(new ErrorHandler('Group with this name already exists', 401))
  }
...
const getMyGroups = TryCatch(async (req, res, next) => {
  ...
  return res.status(200).json({ success: true, message: "Group Created Successfully", chats: groups })
...
const leaveGroup = TryCatch(async (req, res, next) => {
  ...
  emitEvent(req, ALERT, chat.members, `${user.name} has leave the group`);

  return res.status(200).json({ success: true, message: `${user.name} has leave the group` })
...
const deleteGroup = TryCatch(async (req, res, next) => {
  ...
  // here we have to delete all the messages ,chats and attachments or files from the cloudinary
